...
import numpy as np
import torch
from utils.utils_metrics import get_entities_span, classification_report, f1_score, convert_span_to_bio
...
MODEL_CONFIG_CLASSES = list(MODEL_FOR_SPAN_NER_MAPPING.keys())
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        if start_preds is None:
            ...
            start_trues = inputs['start_positions'].detach().cpu().numpy()
            end_trues = inputs['end_positions'].detach().cpu().numpy()
        else:
            ...
            start_trues = np.append(start_trues, inputs['start_positions'].detach().cpu().numpy(), axis=0)
            end_trues = np.append(end_trues, inputs['end_positions'].detach().cpu().numpy(), axis=0)
    ...
    start_preds_list = [[] for _ in range(start_preds.shape[0])]
    end_preds_list = [[] for _ in range(end_preds.shape[0])]
    start_trues_list = [[] for _ in range(start_trues.shape[0])]
    end_trues_list = [[] for _ in range(end_trues.shape[0])]

    for i in range(start_trues.shape[0]):
        for j in range(start_trues.shape[1]):
            if start_trues[i, j] != pad_token_label_id:
                start_preds_list[i].append(span_map[start_preds[i][j]])
                end_preds_list[i].append(span_map[end_preds[i][j]])
                start_trues_list[i].append(span_map[start_trues[i][j]])
                end_trues_list[i].append(span_map[end_trues[i][j]])

    pred_entities = get_entities_span(start_preds_list, end_preds_list)
    true_entities = get_entities_span(start_trues_list, end_trues_list)
    results = {
        "loss": eval_loss,
    ...
    preds_list = convert_span_to_bio(start_preds_list, end_preds_list)
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode):
    ...
    dataset = TensorDataset(all_input_ids, all_input_mask, all_valid_mask, all_segment_ids, all_start_ids, all_end_ids)
    return dataset
...
def main():
    ...
    model = AutoModelForSpanNer.from_pretrained(
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        model = AutoModelForSpanNer.from_pretrained(checkpoint)
        model.to(args.device)
